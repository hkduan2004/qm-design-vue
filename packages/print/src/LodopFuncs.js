 ...
 * @Last Modified time: 2021-10-20 15:16:44
 */
import { QmNotification, QmMessage } from '../../index';
  ...
  try {
    ...
    if (needCLodop()) {
      ...
      if (!LODOP && document.readyState !== 'complete') {
        QmMessage.warning('打印组件还没有加载完毕，请稍等一下再操作');
        return;
      }
      if (!LODOP) {
        QmNotification({
    ...
    const LICENSES = global['print']?.LODOP_LICENSES;
    if (LICENSES?.length >= 2 && LICENSES.every((x) => Array.isArray(x))) {
      LODOP.SET_LICENSES(...LICENSES[0]);
      LODOP.SET_LICENSES(...LICENSES[1]);
    }

    return LODOP;
  } catch (err) {
    QmMessage.error('getLodop 出错:' + err);
  }
}
